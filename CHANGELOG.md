...
### fix
- `navigator.storage.persist()` 요청 추가 — 설치형 PWA에서 고정·알림·진행상태가
  저장소 정리로 사라지던 현상 방지. 캐시 `qh-v24`

---

## [2026-06-18] — 알림 중복 제거 (A/B 동시 발송 방지)

### fix
- 서버 푸시(B) 구독 중이면 앱 열 때 로컬 검사(A)를 건너뜀 — 같은 알림 2번 뜨던 문제 해결
  - `pushService.isPushActive()` 추가 / 캐시 `qh-v23`
...
### perf
- 조회에서 `raw` JSONB 컬럼 제외(프런트 미사용) — 공고/시험 조회 응답 용량 대폭 감소(수 MB↓)
  - `jobService`/`certificationService` 필요한 컬럼만 select

### fix
- 앱 셸 구조로 전환: **헤더·하단 nav 고정 + 본문(.app__view)만 스크롤** (기존엔 문서 전체 스크롤 + nav만 fixed)
...
## [2026-06-18] — 홈 섹션 라벨 높이 통일

### fix
- 개수 칩 유무로 섹션 라벨 높이가 달라(라벨~카드 간격 불일치)지던 문제 — `min-height`로 통일, 간격 정돈
...
## [2026-06-18] — 텍스트 레이아웃 다듬기 (홈 중심)

### style
- 홈 히어로/행 타이포 정리: 줄높이·자간·여백 조정, 제목 강조, 숫자 정렬(tabular-nums)
- 섹션 간격 통일, 페이지 타이틀 강조, "필기시험까지" 띄어쓰기 정리
...
## [2026-06-18] — 핀을 '고정' 토글 칩으로

### style
- 애매하던 핀 아이콘 → **명확한 토글 칩**(`고정` ↔ `고정됨`)으로 교체
- (홈 메인은 고정 항목 중 가장 임박한 것을 하이라이트 — 유지)
...
## [2026-06-18] — 홈 메인=고정 우선 + 핀 비주얼 리디자인

### feat
- 홈 메인 카드(히어로)를 **내가 고정한 항목 우선**으로 (없을 때만 가장 임박한 것)
  - "가까운 일정"보다 "내가 선택/지원한 일정"을 더 중요하게 노출
  - 히어로에 핀 토글 추가

### style
- 핀 강조를 왼쪽 바 → **따뜻한 톤 카드 + 비스듬히 꽂힌 압정(rotate)** 으로 재디자인
...
## [2026-06-17] — 핀 UI 개선 (모바일)

### style
- 핀 버튼을 네모 박스 → **테두리 없는 아이콘**(터치 영역 32px, 고정 시 코랄)
- 고정된 카드/행에 **왼쪽 코랄 강조선**으로 상태를 한눈에
- SW 캐시 `qh-v14` → `qh-v15`

---

## [2026-06-17] — 단계 단순 토글 + 핀(고정) 기능

### feat
- **핀(고정)**: 일정/공고를 고정하면 어느 페이지든 **상단 노출** (`pinService`, localStorage)
  - 홈 상단 "고정" 섹션 + 일정/공고 목록 고정 우선 정렬
  - 정렬(가까운 순)이 밀어내도 중요한 항목을 사용자가 직접 상단 유지
- 단계 타임라인을 **단순 토글**(미입력 ↔ ✓합격)로 변경 (불합격 ✕ 단계 제거)
...
## [2026-06-17] — 홈 히어로/타임라인 라벨 위치 재정렬

### style
- 홈 히어로: 좌(유형·이름·다음단계) / 우(큰 D-day) 좌우 분리로 가독성 개선
- 타임라인 `필기`/`실기` 라벨을 좌측 옆 → **줄 위 헤더**로 이동 (단계 칸 넓게)
